...
public:
    ...
    void trackMets(int n) {
        numMets = n;
    }
    void add(const FuncExp *fe) {
        allValues[fe] = vector< pair< double, double > >();
    }
    int getNumValues(const FuncExp *fe) {
        return allValues[fe].size();
    }
...
void cleanStore(unsigned long *v) {
    delete[] v;
}

void cleanName(LPCSTR *nm) {
    delete[] nm;
}

void cleanState(LPCSTR nm) {
    delete[] nm;
}

void cleanUp(void *vld) {
    delete (SimulatorValidator *)(vld);
}
